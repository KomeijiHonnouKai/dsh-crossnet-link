...
**English: [README.en.md](README.en.md)** —— 英文原文;本页是仓库默认显示的中文原文。
...
> **门禁标记(hygiene gate markers)——请勿删除**:发布集门禁 `.github/scripts/repo-hygiene.ps1` 会逐字检查根 `README.md` 里的下面几段英文,因此它们按**英文原句**保留在此(对应的中文口径见 §1、§7、§12)。
> **Internal analysis and review material does not ship with the repository**(内部评审材料不随仓库发布)。
> **The plugin is read-only and report-only.**
> **License**: **MIT** - settled by the maintainer, so [`LICENSE`](LICENSE) is the final text and not a draft.

**本中文原文与英文原文(`README.en.md`)的逐节对应关系见文末附录:[附录:中文版完整度声明](#附录中文版完整度声明)。**
...
**一句话**:**在 A 电脑的 DSH 里,通过浏览器插件(如 ego-browser)驱动一个已登录的通道页面,直接操作 B 电脑上运行的 DSH —— 两台 DSH 由此形成联动(agent 对 agent)。** 本仓库负责:体检这条链路与前置件是否真的到位、给出只读姿态面板、以及完整干净卸载。它**不负责把链路打通**,它负责判定链路现在处于什么姿态、还缺哪些前置件、以及怎么把它干净地卸掉。

**让该页面在 A 上可达的前置链路是**(只认这一条,已实测跑通的那一条):两端在同一 tailnet + 服务端 `tailscale serve` 把只监听 loopback 的 DSH 端口暴露到 tailnet + 窄 ACL 只放「客户端 → 服务端 `tcp:443`」+ 客户端浏览器带一次性 token 打开一次换长期 cookie。逐条如下:
...
6. 此后 A 上的浏览器插件直接驱动这个(已登录的)通道页面,从而操作 B 上运行的 DSH —— 两台 DSH 联动(agent 对 agent);也正因如此,**能打开那个页面 == 能操作那台机器**(§7)。
...
本节把**这个仓库是怎么做出来的**写清楚:用了什么软件、什么插件、谁做了什么。

**一句话**:本仓库**由 AI 全程实现**(代码、文档、测试与自查),**人负责需求、命名与许可决策、最终验收**,并亲自执行机器级动作。全程只用一个模型:`deepseek-official/deepseek-v4-flash-vision-exp`(DeepSeek V4 Flash)。

1. **运行环境**:DSH Desktop 2.0.13(Electron 应用,界面在 `127.0.0.1:43120`);Windows 10 / Windows 11 + Windows PowerShell 5.1。仓库里所有脚本**零外部依赖** —— 不需要 Node、不需要 Pester、不需要任何模块;`node` 不在 PATH 时预检只打印 `[SKIP]`,退出码不受影响。
2. **模型**:`deepseek-official/deepseek-v4-flash-vision-exp`(DeepSeek V4 Flash)—— 本项目每一次推理都用它,没有换过线。
3. **多智能体怎么组织**:通过 **AgentTeams 插件(`@nanmicoder/dsh-agent-teams`)** 把工作拆成**契约化任务**(每个任务都带 inScope / 验收 / verify 三条命令);角色分工为队长(captain)、研究、工程、测试、技术写作、安全评审、独立验收、发布。两条纪律贯穿始终:**交付物先落盘**,**报告只写可复现的原始输出**。
4. **关键工具链**(软件 / 插件 → 它在本次开发中的作用):

   | 软件 / 插件 | 本次开发中的作用 |
   | --- | --- |
   | `@nanmicoder/dsh-agent-teams` | 队伍编排、任务依赖与成员信箱 |
   | `dsh-ego-browser`、`@yuxianglin/dsh-bridge-browser` | 浏览器桥:在对端机器上做实测;以及本机 GitHub API 被墙时改用浏览器读页面 |
   | `dsh-approval-gate` + `@deepseek-ai/dsh-permission-presets` | 沙箱与审批策略(哪些动作必须由人授权) |
   | `@deepseek-ai/dsh-client-ui-settings-plugins` 与 Slots(`settings.section`、`settings.plugin.item`) | 把面板注册进「设置」的两个座位 |
   | Windows PowerShell 5.1 · git · Python 3.11 + PyYAML(工作流校验)· PSScriptAnalyzer 1.22.0(CI 静态检查)· GitHub Actions(`windows-latest`)· Tailscale(被检链路的软件) | 构建、校验与外部裁判 |
   | 每 5 分钟一次的后台巡检 | 盯长跑任务:仓库状态 / 交付物 / 监听面 / 网络 / 成员活跃度 |

5. **怎么让结论可信**:每个判定都要能用**一条命令复现**;整套件在**冻结快照**上跑(避开并发编辑);对关键断言做**正控**(人为破坏 → 必须变红,证明断言不是空转);并把 **CI 当外部裁判** —— 它确实抓出了 6 条「依赖参考机器」的用例,修法是「夹具里钉死工具 + `-SimulateCleanHost` + 把干净机器模拟接进 CI 常驻门禁」。
6. **人与 AI 的分工**:AI 负责实现代码 / 文档 / 测试并自查;人负责需求、命名与许可决策、最终验收,并**亲自执行机器级动作**(安装插件、改 profile、`git push`、防火墙与电源等改动)。⇒ 本项目**不是「AI 自主发布」**,而是「**AI 实现 + 人类决策与执行**」。
7. **一个容易混的点**:开发**这个仓库**用到的插件(AgentTeams / ego-browser / approval-gate …)**不等于**本仓库**产出**的插件(`remote-tailnet-guard`)。前者是我们做事的工具,后者才是本仓库的交付物。
...
链路有**两端**,每一端都可能各自跑代理 ⇒ 一共四种组合。但**本工具一次只在一台机器上跑,只报那台机器在指定角色下的代理姿态**:客户端看 `BROWSER_PROXY_TSNET`,服务端看 `SERVER_PROXY_STATE` 与 `TAILNET_ROUTE_PRESENT`;对端的代理状态在这一端**观察不到**,所以它从不猜 —— 它只是不出现在这边的判定里。

| # | 客户端代理 | 服务端代理 | ① 客户端会看到什么 | ② 服务端会看到什么 | ③ 判据(命令见下) | ④ 修法 / ⑤ 回滚 | 证据等级 |
|---|---|---|---|---|---|---|---|
| 1 | 关 | 关 | 浏览器直连 `https://<HOST>.<TAILNET_DOMAIN>/`,路径上没有代理;这时打不开的原因**不在代理** | 入站路径与 WinINET 无关 | client `pass/proxy_ok(disabled)`;server `pass/proxy_ok(disabled)` | 无需修;回滚:无 | **推导**(本机不在此格) |
| 2 | 开 | 关 | 浏览器**走代理**:`*.ts.net` 被拿去解析/连接,通常超时或报错;而脚本直接做 TCP 探测**是通的** | 入站路径正常;服务端看不到客户端的代理设置 | client **`degraded/proxy_hijack`**;server `pass` | 客户端:路 1 或路 2;回滚:删掉绕过项 / 把系统代理开关打回原样 | 客户端**实测**;服务端侧**未验证** |
| 3 | 关 | 开 | WinINET 系统代理**不影响入站路径** ⇒ 浏览器照常打得开;若服务端跑的是 **TUN 模式**,则表现为 `timeout` | `SERVER_PROXY_STATE` = **`degraded/proxy_active_route_intact`**(`confidence=low`);TUN 模式下还会 `TAILNET_ROUTE_PRESENT` = `blocked/route_tailnet_missing` | server 两项(命令见下) | 只有 TUN 模式才需要处理:服务端走路 1;回滚:把代理恢复原样 | 服务端**实测**;TUN 分支**未验证** |
| 4 | 开 | 开 | 与第 2 格完全相同(浏览器这一端只看客户端的代理) | 与第 3 格完全相同 | client + server **两项都 `degraded`** | 两端各按路 1 / 路 2 | **实测(两端)** —— 写这一节时本机就在这一格 |

> **本机(写这份 README 的机器)的真实状态**,不是举例:`ProxyEnable=1`、`ProxyServer=127.0.0.1:7897`、`ProxyOverride` 里既没有 `ts.net` 也没有 `100.64.` ⇒ 本机正落在**第 4 格**;实测 `BROWSER_PROXY_TSNET = degraded/proxy_hijack`、`SERVER_PROXY_STATE = degraded/proxy_active_route_intact`。**其余三格没有一格被声称端到端验证过**:第 2、3 格各只有一半来自本机实测(另一半是对端状态,这里观察不到),第 1 格是推导。

**③ 怎么判:三条命令,一条一条粘**

```powershell
# ① 这一端开不开代理(客户端与服务端各自跑一次;ProxyEnable=1 就是开着)
Get-ItemProperty 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Internet Settings' |
  Select-Object ProxyEnable, ProxyServer, ProxyOverride

# ② tailnet 路由:在表里找 100.64.0.0/10 那一行(掩码 255.192.0.0)
route print -4

# ③ 本工具的判定:两台机器上各跑一次,角色别写反
powershell -NoProfile -ExecutionPolicy Bypass -File src/collect.ps1 -CheckOnly -Role client
...
读法:`ProxyEnable=1` ⇒ 系统代理开着;`ProxyOverride` 是**分号分隔的通配字符串**,里面**没有** `ts.net`、也没有 `100.64.` ⇒ 浏览器访问 tailnet 页面会走代理,而那个代理多半不认识 `*.ts.net`。`ProxyEnable=0` 时后面两个值即使还在也不生效。

**② 的原始输出(本机实测,只留 tailnet 相关行;本机/对端地址用本仓库的占位符代替)**
...
⚠️ 本机实测还有一条必须记住的经验:**表里没有 `100.64.0.0/10` 聚合行,链路照样是通的**。这台机器只有若干 /32 主机路由(本机 tailnet 地址、对端地址,以及 tailnet 自己的 DNS 地址),而同一时刻 `-Peer` 探测实测 `connected`(15~19 ms)。所以 `TAILNET_ROUTE_PRESENT` 报 `blocked/route_tailnet_missing` 时要当**线索**读,不要单独据此宣布链路断了:先看下面 ④ 的三态,再看对应那一格的修法。

**③ 的原始输出(本机实测;界面语言为中文,英文界面会输出对应的英文行)**
...
`-AsJson` 下这两项的 `verdict`/`reason` 分别是 `degraded`/`proxy_hijack` 与 `degraded`/`proxy_active_route_intact`;`raw.confidence=low` 表示服务端那一项**只**能证明「系统代理开着」,证明不了它影响不影响入站。

**④ 连通性三态(客户端侧,`-Peer` 指向服务端)** —— 三种结果本机都实测过,原文样式如下:

```text
# connected:TCP 握手成功(本机实测 15~19 ms)—— 只证明 TCP 层可达,不证明 TLS/证书
[PASS] PEER_TCP_443 (client) - 对端 443 的 TCP 三态
    TCP handshake to the peer's 443 succeeded (19 ms) - TCP-layer reachable (TCP only, no TLS/HTTP).

# refused:路径通、443 上没人听(serve 没跑 / 端口不对)(本机实测 2043 ms)—— 与代理无关
[BLOCKED] PEER_TCP_443 (client) - 对端 443 的 TCP 三态
    The peer actively refused 443 (2043 ms) - path works but nothing listens on 443 (serve not running or wrong port).

# timeout:包被丢掉(ACL/防火墙),或对端睡着了(本机实测 3012 ms)—— 代理劫持也长这样
[BLOCKED] PEER_TCP_443 (client) - 对端 443 的 TCP 三态
...
三态怎么用:**connected** ⇒ TCP 层没问题,页面还打不开就去查代理(第 2、4 格);**refused** ⇒ 与代理无关,是 serve / 端口的问题;**timeout** ⇒ 先怀疑代理劫持与服务端路由(第 3、4 格),再怀疑 ACL 与对端电源。

**三个容易踩的点**

1. **WinINET 的 `ProxyOverride` 不支持 CIDR。** 要绕过得写 `*.ts.net;100.64.*`;**写 `100.64.0.0/10` 不生效** —— 这一栏只认分号分隔的通配字符串,不认网段语法。注意采集器的判据只做**字符串匹配**(`ts.net` 或 `100.64.`),所以写 `100.64.0.0/10` 会让它报 `pass`,而浏览器**仍然不走直连** —— 判据通过不等于浏览器真的绕过,以页面能不能打开为准。
2. **git 的代理与系统代理是两件事。** git 读 `git config --global http.proxy`(或 `HTTP_PROXY` / `HTTPS_PROXY` 环境变量),**不读** `ProxyEnable`。所以「关掉系统代理让浏览器直连 tailnet」与「git 照旧走代理推 GitHub」可以同时成立:关系统代理**不会**顺手断开 git,反之也一样。
3. **TUN / 透明代理模式下读不到第三方代理软件的内部规则。** 它能接管路由,而本工具只能证明「`route print -4` 里那条聚合路由不见了」这一半;是不是它干的、它的规则放不放 `100.64.0.0/10`,**都判不了** ⇒ 如实落 `unknown`(相关项报 `route_tailnet_missing`,归因写「不可判定」),**不猜**。

**④⑤ 每一格的修法与回滚:两条路**

**路 1(推荐,一次性):把 tailnet 设为直连(DIRECT)。**
- 客户端:在**代理软件自己的**规则里把 `*.ts.net` 与 `100.64.0.0/10` 设为直连(各家叫法不同:「绕过」「直连规则」「自定义规则」)。若你只能改 Windows 系统代理那一栏,就写 `*.ts.net;100.64.*`(见易踩点 1)。
- 服务端:只有 **TUN 模式**才需要动 —— 让 `100.64.0.0/10` 不走代理隧道。**WinINET 系统代理不影响入站路径**,为了入站去关它没有意义。
- 影响面:只把 tailnet 流量排除在代理之外,其它流量照旧走代理。
- **回滚**:删掉你加进去的那两条直连规则(系统代理那一栏就是删掉 `*.ts.net;100.64.*` 这一段)。

**路 2(最省事):用链路时关掉系统代理,用完再开。**
- 客户端:设置 → 网络和 Internet → 代理 → 关掉「使用代理服务器」。
- 服务端:同样只对 TUN 模式有意义;WinINET 那一栏可以不管。
- 影响面:关掉期间**所有**浏览器流量直连 —— 依赖代理才能访问的站点会打不开。
- **回滚**:把开关打回原样,原来的 `ProxyEnable` / `ProxyServer` / `ProxyOverride` 值照抄回去 —— **改之前先把 `Get-ItemProperty …` 的输出留着**,它就是你的备份。

> 本工具**从不修改**任何代理设置,所以它也没有回滚动作;上面两条路里改设置的每一步都是**你**做的,回滚也由你做。
...
**两个 README 都随发布集发布**:根 `README.md` 是中文原文(GitHub 默认显示这一份),`README.en.md` 是它的英文原文;两份的章节编号、顺序与层级一一对应,也都登记进卫生门禁的默认扫描根,因此谁都不会漂到门禁覆盖范围之外。
...
许可已定(MIT,`Copyright (c) 2026 KomeijiHonnouKai` —— 已反映在 `LICENSE`、两份 README 与 `CHANGELOG.md` 里)。**仓库名也已定**:就是本文件顶部 clone URL 里的那个(改名仍是每个文件一行的编辑)。**账号也已填好**(`KomeijiHonnouKai`,见 clone URL)。仍然属于你的事:在 GitHub 上创建那个仓库并做第一次 `git push`。按政策不附任何二进制。
...
| 常驻包只读预检(0/1/2) | `panel/plugin-preflight.ps1` | 在位;实测 `checks: 66  passed: 66  failed: 0  skipped: 3`,exit 0(本修订版重跑) |
...
| `LICENSE` / `README.md` / `README.en.md` / `SECURITY.md` / `CHANGELOG.md` / `CONTRIBUTING.md` | 仓库根 | 在位 |
...
这份 `README.md` 是中文原文(仓库默认显示的那一份),英文原文 `README.en.md` 与它**逐节对应**:章节编号、顺序与层级与英文原文一一对应,`# remote-tailnet-guard` 标题、头部条目(Status / License / Repository / Platform / Security policy)、§1–§12、以及 Part II 的 §13–§21 全部在位,**没有任何一节被静默删掉**。

下表逐节点名每一节的处理方式。只有**维护者过程记录类**的节做了**忠实压缩**(把逐次时间线、逐轮毫秒数、内部评审过程压成结论),其余节是全文翻译;压缩的节都给出了指向英文原文的锚点(`README.en.md` + 节号)。压缩的含义是「信息不删、细节减写」:结论、命令、判据、边界、未验证项都保留;被压掉的只是重复的过程叙述。
...
| §8 仓库布局与发布集 | 全文翻译 **+ 两处新增登记** | 布局块与发布集清单加入中文版 README(t42)与 `plugin/` + `panel/plugin-preflight.ps1` + `docs/install/plugin-package.md`(t44) |
...
| §19 未决项与刻意的决定 | **忠实压缩** | 见 `README.en.md` §19:19.1 的七条判断每条保留一句,19.2 的表(含「已关闭」注记)逐行保留 |
| §20 交付清单与陌生人需要的三条命令 | 全文翻译 | 清单表逐行 + 三条命令原样 |
| §21 卸载与残留 | 全文翻译 | 六步表 / 四种分类 / 退出码 / 承诺边界逐条 |

**跨文档一致性**:本文件里的文件路径、命令、参数名与 `README.en.md` 逐个对齐。核对方式与结果:把本文件里所有以 `-` 开头的参数抽出来(41 个去重值),与仓库内 7 个脚本(采集器、前置件检查器、卸载器、三个测试入口、卫生门禁)以及它们引用的外部脚本(skill 的 `verify.ps1` / `server-setup.ps1`、`.dsh/tools/sync-skill.ps1`)的 `param()` 声明、加上实际 cmdlet 的参数集逐个比对 —— **invented = 0**;本文件引用的仓库相对路径逐个 `Test-Path` 核对 —— **全部存在**(含 `i18n/labels.{en,zh}.json` 花括号写法展开的两个文件)。两份 README 的〈目的〉〈GUI〉〈实现方式与作者〉三节内容一致,只是语言不同。若两份文件出现不一致,**以英文 `README.en.md` 为准**,并把本文件视为需要修的那一份。
