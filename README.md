...
**本仓库做什么**：给两台 DSH 之间的跨网链路做只读体检（链路姿态与前置件）、
提供一张只读面板，以及把装上的实验插件完整卸载；只读、只报，
...
- **P1** 不新增网络监听、网络路由与防火墙规则；只在既有 web 服务上注册一条只读
  HTTP 路由（见 P9），监听面不扩大。
- **P2** 默认只读；写系统面的组件只有卸载器，且默认干跑。插件首次加载会经
  auto-config 写 settings.yaml 的 4 个键，这是唯一的配置副作用（见 §5 安装文档）。
- **P3** 不做自动写入，`-Apply` 被拒绝。
- **P4** 不读、不打印、不存储、不外传凭据。
- **P5** 无遥测、无回调、无自动更新。
...
- **P8** 面板是只读展示，没有写入按钮。
- **P9** 在既有 web 服务上注册的路由不经过 DSH 的 web 认证：DSH 默认只监听 loopback，
  这时它仅本机可达；一旦用 tailscale serve 把 DSH 端口发布到 tailnet，这条路由随之对
  tailnet 可达（同源校验对缺 Origin 头的对端无效）。它只读、只返回体检结果、不读凭据，
  不是 RCE；风险是资源消耗面——对端可反复触发采集器。
